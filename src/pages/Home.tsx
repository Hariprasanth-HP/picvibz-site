      ...
      <div>
        <div className="relative rounded-2xl overflow-hidden h-48 mb-3 shadow-lg">
          {ads.map((ad, index) => (
            <div
              key={ad.id}
              className={cn('absolute inset-0 transition-opacity duration-1000', index === currentAdIndex ? 'opacity-100 z-10' : 'opacity-0 z-0')}
            >
        ...
        <div className="flex justify-center gap-1.5">
          {ads.map((_, index) => (
            <div
              key={index}
              className={cn('h-1 rounded-full transition-all duration-300', index === currentAdIndex ? 'w-4 bg-[#a855f7]' : 'w-1 bg-gray-600')}
            ></div>
          ))}
        </div>
      </div>
