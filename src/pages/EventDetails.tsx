...
import {
  ...
  ArrowUpDown, Filter, LayoutGrid, Check, FolderDown, UserPlus, FileDown,
  Sparkles, Search, Tag, Pencil, Plus, Camera as CameraIcon
} from 'lucide-react';
...
export function EventDetails() {
  ...
  return <EventDetailsContent id={id as string} event={event} />;
}

function EventDetailsContent({ id, event }: { id: string; event: AppEvent }) {
  const navigate = useNavigate();
  const { deletePhoto, deletePhotos, setEventPhotos, updatePhotoDetails, autoUploadEvents, toggleAutoUploadForEvent } = useAppContext();
  const { user } = useAuth();
  ...
  const memberStats = useMemo(() => {
    ...
    event.photos.forEach(p => {
      ...
      uploaderMap.set(p.uploader, {
        ...
        role: p.uploader === 'John Doe' || p.uploader === 'Admin' ? 'Admin' : 'Member'
      });
    });
  ...
  const filteredAndSortedPhotos = useMemo(() => {
    ...
    if (semanticFilter !== 'all') {
      if (semanticFilter === 'just_you' && user) {
        result = result.filter(p => p.uploaderId === user.uid || p.detectedPeople?.some(person => person.name === user.displayName));
  ...
  const canDelete = (photo: Photo) => {
    ...
    return user.role === 'admin' || user.uid === photo.uploaderId;
  };
  ...
  return (
    <div className="h-full flex flex-col bg-[#0A0A0A] overflow-y-auto pb-24 md:pb-0 text-white">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex gap-4">
          ...
          <Button
            ...
            className="flex-1 bg-[#a855f7] rounded-2xl py-3.5 flex items-center justify-center gap-2 font-bold hover:opacity-90 hover:bg-[#a855f7] h-auto"
          >
            <CameraIcon size={20} />
            Camera
          </Button>
          <Button
            ...
            className="flex-1 bg-[#1A1A1A] border border-white/5 rounded-2xl py-3.5 flex items-center justify-center gap-2 font-bold hover:bg-[#2A2A2A] h-auto"
          >
            <Upload size={20} />
            Upload
          </Button>
          ...
          {showSmartUpload && viewMode === 'assets' && (
            ...
            >
              ...
              <div className="flex items-center gap-3 relative z-10">
                <Button
                  onClick={() => fileInputRef.current?.click()}
                  className="bg-[#a855f7] text-white px-4 py-2 rounded-xl text-xs font-bold hover:opacity-90 hover:bg-[#a855f7] h-auto"
                >
                  Upload All
                </Button>
                <button
          ...
          {viewMode === 'members' ? (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-[#1A1A1A] border border-white/5 rounded-[2rem] overflow-hidden">
                  {memberStats.members.map((member, idx) => (
                    <div
                      key={member.name}
                      className={`flex items-center justify-between p-5 hover:bg-white/5 transition-colors ${idx !== memberStats.members.length - 1 ? 'border-b border-white/5' : ''}`}
                    >
              ...
              <div className="bg-gradient-to-br from-[#a855f7]/10 to-[#3b82f6]/10 border border-white/5 p-8 rounded-[2.5rem] text-center">
                ...
                <Button
                  onClick={() => setShowInviteModal(true)}
                  className="bg-white text-black px-8 py-3 rounded-2xl text-xs font-black uppercase tracking-widest hover:opacity-90 hover:bg-white h-auto"
                >
                  Send Invite
                </Button>
              </div>
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="relative">
                ...
                <Input
                  ...
                  className="h-auto w-full bg-[#1A1A1A] border border-white/5 rounded-2xl py-4 pl-12 pr-4 text-sm focus:border-[#a855f7]"
                />
              </div>
      ...
      {/* Media Details Modal */}
      <Dialog open={!!selectedPhoto} onOpenChange={(open) => { if (!open) { setSelectedPhoto(null); setIsEditingPhoto(false); } }}>
        {selectedPhoto && (
                ...
                <div className="space-y-6">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              ...
                              <Input
                                ...
                                className="h-auto w-full bg-black/50 border border-white/10 rounded-lg px-3 py-2 text-sm focus:border-[#a855f7]"
                            ...
                            <div>
                              ...
                              <select
                                ...
                                className="w-full bg-black/50 border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#a855f7] appearance-none text-white"
                              >
                            ...
                            <div>
                              ...
                              <Input
                                ...
                                className="h-auto w-full bg-black/50 border border-white/10 rounded-lg px-3 py-2 text-sm focus:border-[#a855f7] [color-scheme:dark]"
                              />
                            </div>
                          </div>
                        </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <Input
                      ...
                      className="h-auto w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-[#a855f7]"
      ...
      <Sheet open={showFilters} onOpenChange={setShowFilters}>
        <SheetContent side="right" className="w-full max-w-sm bg-[#111] border-l border-white/10 p-0 flex flex-col">
          <SheetHeader className="flex-row items-center justify-between p-6 border-b border-white/10 space-y-0">
          ...
          <div className="p-6 border-t border-white/10 bg-black/20">
            <Button
              ...
              variant="outline"
              className="w-full py-3.5 rounded-xl border border-white/10 text-sm font-bold text-gray-300 hover:bg-white/5 hover:text-white h-auto"
            >
              Reset All Filters
            </Button>
          </div>
      ...
      <Dialog open={showSettingsModal} onOpenChange={setShowSettingsModal}>
        <DialogContent className="bg-[#1A1A1A] border border-white/10 rounded-3xl p-6 w-full max-w-md text-white">
          <DialogHeader>
          ...
          <div className="space-y-4">
            <div className="p-4 bg-white/5 rounded-2xl border border-white/5">
              <div className="flex items-center justify-between mb-2">
                ...
                <Switch
                  ...
                  onCheckedChange={(val) => toggleAutoUploadForEvent(event.id, val)}
                />
              </div>
            ...
            <div className="p-4 bg-white/5 rounded-2xl border border-white/5">
              ...
              <Button
                ...
                className="w-full bg-red-500/10 text-red-500 hover:bg-red-500/20 hover:text-red-500 border border-red-500/20 rounded-xl py-3 flex items-center justify-center gap-2 font-bold h-auto"
              >
                <Trash2 size={18} />
      ...
      <Dialog open={showInviteModal} onOpenChange={setShowInviteModal}>
        <DialogContent className="bg-[#1A1A1A] w-full max-w-md rounded-3xl overflow-hidden border border-white/10 flex flex-col p-0 gap-0 text-white">
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="bg-black/40 rounded-2xl p-4 border border-white/5 flex items-center gap-3">
              ...
              <Button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-bold h-auto ${copied ? 'bg-green-500 text-white hover:bg-green-500' : 'bg-[#a855f7] text-white hover:bg-[#a855f7] hover:opacity-90'}`}
              >
                {copied ? 'Copied!' : 'Copy'}
              </Button>
            </div>
