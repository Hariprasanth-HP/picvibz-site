...
type PayMethod = 'gpay' | 'phonepe' | 'paytm' | 'card' | 'netbanking';

export function Payment() {
  const navigate = useNavigate();
  const location = useLocation();

  const amount = location.state?.amount || 499;
      ...
      <div className="p-6 md:max-w-2xl md:mx-auto md:w-full">
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="bg-white dark:bg-[#1A1A1A] rounded-2xl border border-gray-200 dark:border-white/5 overflow-hidden shadow-sm">
              <label className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-white/5 cursor-pointer hover:bg-gray-50 dark:hover:bg-[#2A2A2A] transition-colors">
                ...
                <input type="radio" className="sr-only" name="payMethod" checked={selectedMethod === 'gpay'} onChange={() => setSelectedMethod('gpay')} />
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${selectedMethod === 'gpay' ? 'border-[#a855f7]' : 'border-gray-300 dark:border-gray-600'}`}>
                  {selectedMethod === 'gpay' && <div className="w-2.5 h-2.5 rounded-full bg-[#a855f7]" />}
                </div>
              </label>
              ...
              <label className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-white/5 cursor-pointer hover:bg-gray-50 dark:hover:bg-[#2A2A2A] transition-colors">
                ...
                <input type="radio" className="sr-only" name="payMethod" checked={selectedMethod === 'phonepe'} onChange={() => setSelectedMethod('phonepe')} />
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${selectedMethod === 'phonepe' ? 'border-[#a855f7]' : 'border-gray-300 dark:border-gray-600'}`}>
                  {selectedMethod === 'phonepe' && <div className="w-2.5 h-2.5 rounded-full bg-[#a855f7]" />}
                </div>
              </label>
              ...
              <label className="flex items-center justify-between p-4 cursor-pointer hover:bg-gray-50 dark:hover:bg-[#2A2A2A] transition-colors">
                ...
                <input type="radio" className="sr-only" name="payMethod" checked={selectedMethod === 'paytm'} onChange={() => setSelectedMethod('paytm')} />
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${selectedMethod === 'paytm' ? 'border-[#a855f7]' : 'border-gray-300 dark:border-gray-600'}`}>
                  {selectedMethod === 'paytm' && <div className="w-2.5 h-2.5 rounded-full bg-[#a855f7]" />}
                </div>
              </label>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="bg-white dark:bg-[#1A1A1A] rounded-2xl border border-gray-200 dark:border-white/5 overflow-hidden shadow-sm">
              <button onClick={() => setSelectedMethod('card')} className="w-full flex items-center justify-between p-4 border-b border-gray-200 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-[#2A2A2A] transition-colors text-left">
                <div className="flex items-center gap-4">
              ...
              <button onClick={() => setSelectedMethod('netbanking')} className="w-full flex items-center justify-between p-4 hover:bg-gray-50 dark:hover:bg-[#2A2A2A] transition-colors text-left">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-gray-100 dark:bg-[#2A2A2A] flex items-center justify-center">
        ...
        <div className="mt-8">
          <Button
            onClick={() => navigate('/payment-success', { state: { amount, eventName, method: selectedMethod } })}
            className="w-full bg-gradient-to-r from-[#8b5cf6] to-[#d946ef] rounded-xl py-4 font-bold text-lg text-white shadow-[0_0_20px_rgba(139,92,246,0.3)] flex items-center justify-center gap-2 hover:opacity-90 hover:from-[#8b5cf6] hover:to-[#d946ef] h-auto"
          >
            Pay ₹{amount}.00
